...
export default class Button extends EventEmitter {
  constructor(context) {
    ...
    this.sm = new StateMachine('Button', 'Idle', Button.stateMap, context);

    setWatch(() => this.sm.handleEvent('_button_pressed'), BTN, {
      repeat: true,
      edge: 'rising',
      debounce: 50,
    });
    setWatch(() => this.sm.handleEvent('_button_released'), BTN, {
      repeat: true,
      edge: 'falling',
      debounce: 50,
    });
...
Button.stateMap = {
  ...
  _PressDetected: {
    ...
    entry: function (handleEvent) {
      return setTimeout(() => handleEvent('_timeout_longClick'), midDuration);
    },
    exit: clearTimeout,
  },
  _ReleaseBeforeTimeout: {
    ...
    entry: function (handleEvent) {
      return setTimeout(() => handleEvent('_timeout_click'), shortDuration);
    },
    exit: clearTimeout,
  },
  _SecondPressDetected: {
    ...
    entry: function (handleEvent) {
      return setTimeout(
        () => handleEvent('_timeout_longDoubleClick'),
        midDuration
      );
    },
    exit: clearTimeout,
  ...
  _ReleaseSecondPressBeforeTimeout: {
    ...
    entry: function (handleEvent) {
      return setTimeout(
        () => handleEvent('_timeout_doubleClick'),
        shortDuration
      );
    },
    exit: clearTimeout,
  ...
  _ThirdPressDetected: {
    ...
    entry: function (handleEvent) {
      return setTimeout(
        () => handleEvent('_timeout_longTripleClick'),
        midDuration
      );
    },
    exit: clearTimeout,
  ...
  _ReleaseThirdPressBeforeTimeout: {
    ...
    entry: function (handleEvent) {
      return setTimeout(
        () => handleEvent('_timeout_tripleClick'),
        shortDuration
      );
    },
    exit: clearTimeout,
